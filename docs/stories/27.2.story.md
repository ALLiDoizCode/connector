...
- [x] Task 5: Verify test suite passes (AC: 6)
  - [x] 5.1 Run `npm test --workspace=@agent-society/connector` — all tests must pass
  - [x] 5.2 Specifically verify `connector-node-optional-deps.test.ts` passes
  - [x] 5.3 Run `npm test --workspace=@agent-society/shared` — shared package unaffected

- [x] Task 6: Verify no coverage regression (AC: 7)
  - [x] 6.1 Run `npm test --workspace=@agent-society/connector -- --coverage` if available
...
- **Key Commands:**
  - `npm test --workspace=@agent-society/connector` — all connector tests
