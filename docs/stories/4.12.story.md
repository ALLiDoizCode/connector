...
- [x] Task 6: Update Documentation with Production Deployment Guide (AC: 17)
  ...
  - [ ] Document BTP peer configuration:
    ...
    - [ ] How to configure BTP_PEER_*_SECRET environment variables
  ...
  - [ ] Document troubleshooting:
    ...
    - [ ] BTP authentication fails: Verify BTP_PEER_*_SECRET matches peer authToken
...
- Environment variable injection (CONFIG_FILE, NODE_ID, LOG_LEVEL, BTP_PEER_*_SECRET)
...
- Environment variable naming conventions (UPPER_SNAKE_CASE, BTP_PEER_<ID>_SECRET)
...
6. **Environment variable pattern**: ✓ Follows NODE_ID, LOG_LEVEL, BTP_PEER_*_SECRET pattern
...
- ✓ ALL secrets in .env file (not YAML) via ${BTP_PEER_*_SECRET:-} placeholders
