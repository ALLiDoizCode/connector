...
- [x] Task 6: Verify all existing tests still pass (AC: 7)
  - [x] 6.1 Run `npm test --workspace=@agent-runtime/connector` — all existing tests must pass (test environment has all deps installed)
  - [x] 6.2 Run `npm test --workspace=@agent-runtime/shared` — shared package unaffected
...
- [x] Task 7: Update package.json and verify dependency count (AC: 1, 6)
  - [x] 7.1 Final `packages/connector/package.json` should have ≤ 10 direct `dependencies` entries:
    - `@agent-runtime/shared`, `ws`, `pino`, `tslib`, `zod`, `js-yaml` (6 confirmed core)
  ...
  - [x] 7.5 Run `npm ls --workspace=@agent-runtime/connector --depth=0` to verify dependency tree is correct
...
This is the first story in Epic 26 (npm Publishing Readiness). This epic prepares `@agent-runtime/shared` and `@agent-runtime/connector` for npm publication. Story 26.1 focuses specifically on trimming the connector's dependency footprint so that `npm install @agent-runtime/connector` doesn't pull in 30+ packages for consumers who only need the core ILP connector functionality.
...
|---------|---------|-------|-------|
| `@agent-runtime/shared` | `*` | ILP types, OER codec | Throughout |
...
- **Key test commands:**
  - `npm test --workspace=@agent-runtime/connector` — all tests
...
1. **AC1** ✓ — 6 direct dependencies (≤10 target met): `@agent-runtime/shared`, `js-yaml`, `pino`, `tslib`, `ws`, `zod`
