...
- [x] Task 3: Add `ILPPreparePacket` re-export from `@agent-society/shared` (AC: 4)
  - [x] 3.1 Add `ILPPreparePacket` to the existing `export type { ILPFulfillPacket, ILPRejectPacket } from '@agent-society/shared'` line in `lib.ts` — resulting in `export type { ILPPreparePacket, ILPFulfillPacket, ILPRejectPacket } from '@agent-society/shared'`
  - [x] 3.2 `ILPPreparePacket` is already exported from `@agent-society/shared` at `packages/shared/src/types/ilp.ts:60` — no changes needed in the shared package
...
- [x] Task 7: Verify `npm pack` includes all `.d.ts` files (AC: 6)
  ...
  - [x] 7.2 Run `npm pack --workspace=@agent-society/connector --dry-run` and verify the output includes `dist/lib.d.ts` and all relevant `.d.ts` files for exported modules
...
- From `@agent-society/shared`: `ILPFulfillPacket`, `ILPRejectPacket`
...
4. `ILPPreparePacket` — already exported from `@agent-society/shared` at `packages/shared/src/types/ilp.ts:60`, just needs re-export from lib.ts
