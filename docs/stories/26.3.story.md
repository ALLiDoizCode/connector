...
- [x] Task 2: Add `prepublishOnly` hooks to both package.json files (AC: 3)
  ...
  - [x] 2.4 Verify `prepublishOnly` runs correctly by doing a `npm pack` in each workspace (pack triggers prepublishOnly) — `npm pack --workspace=@agent-society/shared --dry-run` and `npm pack --workspace=@agent-society/connector --dry-run`
...
- [x] Task 3: Create package validation script `scripts/validate-packages.mjs` (AC: 2, 4, 5, 6, 7)
  ...
  - [x] 3.3 Step 2 — Pack both packages: run `npm pack --workspace=@agent-society/shared` and `npm pack --workspace=@agent-society/connector` in root, capturing tarball filenames from stdout
  ...
  - [x] 3.14 Step 13 — Verify connector `bin` entry: check that `node_modules/.bin/agent-runtime` exists in the temp dir (or the resolved path `node_modules/@agent-society/connector/dist/cli/index.js` exists) — confirms the CLI entry point is included in the published tarball
  - [x] 3.15 Step 14 — Verify no circular dependencies: check that `npm ls` in temp dir shows clean dependency tree with `@agent-society/shared` appearing only once (as connector's dependency and as direct install, not circular)
...
- [x] Task 5: Verify existing tests still pass (AC: 6)
  - [x] 5.1 Run `npm test --workspace=@agent-society/shared` — all shared tests must pass (170/170 passed)
  - [x] 5.2 Run `npm test --workspace=@agent-society/connector` — connector test results match base branch (93 suites passed, 1908 tests passed; 42 suites failed due to pre-existing optional dep migration type errors)
...
- [x] Task 6: Verify publish scripts work end-to-end with dry run (AC: 1)
  - [x] 6.1 Run `npm publish --workspace=@agent-society/shared --dry-run --access public` — succeeds (builds, tests, shows what would be published)
  - [x] 6.2 Run `npm publish --workspace=@agent-society/connector --dry-run --access public` — prepublishOnly fires correctly; build:publish succeeds but npm test fails due to pre-existing optional dep type errors (same as base branch). This correctly prevents accidental publishing when tests fail.
...
- `@agent-society/shared` dependency in connector changed from `"*"` to `"^1.0.0"`
...
- 6 direct deps: `@agent-society/shared`, `js-yaml`, `pino`, `tslib`, `ws`, `zod`
...
`@agent-society/connector` depends on `@agent-society/shared` at `"^1.0.0"`. When publishing:
...
- **Key validation commands:**
  ...
  - `npm test --workspace=@agent-society/connector` — connector tests
  - `npm run validate-packages` — full validation flow
  - `npm publish --workspace=@agent-society/shared --dry-run --access public` — dry-run shared publish
  - `npm publish --workspace=@agent-society/connector --dry-run --access public` — dry-run connector publish
