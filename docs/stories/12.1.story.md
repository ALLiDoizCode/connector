...
- [x] Task 3: Audit for any other "M2M Explorer" or "M2M" references in explorer-ui (AC: 1)
  ...
  - [x] Note: `src/lib/event-types.ts` line 4 contains a comment referencing `@agent-society/shared` — this refers to the npm package namespace, not UI branding. Update comment to `@agent/shared` or similar only if the package is being renamed; otherwise leave as-is since it reflects the actual import path.
...
- Left `@agent-society/shared` references in event-types.ts comments as-is — they reference the actual npm package namespace, not UI branding
