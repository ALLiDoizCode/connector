...
- [ ] Task 7: Implement Comprehensive Audit Logging (AC: 7)
  - [ ] Create AuditLogger class
    - [ ] File: `packages/connector/src/security/audit-logger.ts`
    - [ ] Constructor parameters: logger (Pino), config (AuditLogConfig)
    - [ ] [Source: Epic 12 Story 12.2 AC 7 audit logging requirement]
  - [ ] Define audit log event types
    - [ ] Events: SIGN_REQUEST, SIGN_SUCCESS, SIGN_FAILURE, KEY_ROTATION_START, KEY_ROTATION_COMPLETE, KEY_ACCESS_DENIED
    - [ ] AuditLogEntry interface: { event, keyId, timestamp, nodeId, backend, details? }
    - [ ] [Source: Epic 12 Story 12.2 audit log structure]
  - [ ] Implement audit log methods
    - [ ] Method: `logSignRequest(keyId: string, messageHash: string): void`
    - [ ] Method: `logSignSuccess(keyId: string, signature: string): void`
    - [ ] Method: `logSignFailure(keyId: string, error: Error): void`
    - [ ] Method: `logKeyRotation(oldKeyId: string, newKeyId: string): void`
    - [ ] Method: `logAccessDenied(keyId: string, reason: string): void`
    - [ ] All methods write structured JSON to Pino logger
    - [ ] [Source: docs/architecture/error-handling-strategy.md lines 18-48]
  - [ ] Integrate audit logger into KeyManager
    - [ ] In KeyManager.sign(): Call auditLogger.logSignRequest() before signing, logSignSuccess() or logSignFailure() after
    - [ ] In KeyManager.rotateKey(): Call auditLogger.logKeyRotation() with old and new key IDs
    - [ ] Pattern: `this.auditLogger.log(event, { keyId, timestamp: Date.now(), nodeId: this.config.nodeId, backend: this.config.backend, details });`
    - [ ] [Source: docs/prd/epic-12-multi-chain-settlement-production-hardening.md lines 240-247]
  - [ ] Implement Pino serializers for sensitive data redaction
    - [ ] Redact fields: privateKey, PIN, credentials
    - [ ] Example: `pino({ serializers: { credentials: () => '[REDACTED]', PIN: () => '[REDACTED]' } })`
    - [ ] [Source: docs/architecture/security.md lines 66-76, docs/architecture/coding-standards.md line 24]
  - [ ] Implement audit log export
    - [ ] Method: `exportAuditLogs(startDate: number, endDate: number): Promise<AuditLogEntry[]>`
    - [ ] Read audit logs from Pino output (JSON lines format)
    - [ ] Filter logs by date range
    - [ ] Return array of audit log entries for compliance reporting
    - [ ] [Source: Epic 12 Story 12.2 audit log retention and export requirement]
  - [ ] Unit test: Verify audit log written for SIGN_REQUEST event
  - [ ] Unit test: Verify audit log written for SIGN_SUCCESS event
  - [ ] Unit test: Verify audit log written for KEY_ROTATION_COMPLETE event
  - [ ] Unit test: Verify sensitive data (PIN, credentials) redacted in logs

- [ ] Task 8: Add Environment Variable Configuration (AC: 8)
  - [ ] Define KeyManagerConfig environment variables
    - [ ] Variables: KEY_BACKEND (env | aws-kms | gcp-kms | azure-kv | hsm)
    - [ ] AWS: AWS_REGION, AWS_KMS_EVM_KEY_ID, AWS_KMS_XRP_KEY_ID, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    - [ ] GCP: GCP_PROJECT_ID, GCP_LOCATION_ID, GCP_KEY_RING_ID, GCP_KMS_EVM_KEY_ID, GCP_KMS_XRP_KEY_ID
    - [ ] Azure: AZURE_VAULT_URL, AZURE_EVM_KEY_NAME, AZURE_XRP_KEY_NAME, AZURE_TENANT_ID, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET
    - [ ] HSM: HSM_PKCS11_LIBRARY_PATH, HSM_SLOT_ID, HSM_PIN, HSM_EVM_KEY_LABEL, HSM_XRP_KEY_LABEL
    - [ ] [Source: docs/prd/epic-12-multi-chain-settlement-production-hardening.md lines 293-310]
  - [ ] Update ConnectorConfig type
    - [ ] File: `packages/connector/src/config/types.ts`
    - [ ] Add field: `security: { keyManagement: KeyManagerConfig }`
    - [ ] [Source: Epic 12 Story 12.2 AC 8 configuration requirement]
  - [ ] Implement configuration loading from environment variables
    - [ ] Function: `loadKeyManagerConfig(): KeyManagerConfig`
    - [ ] Read KEY_BACKEND from process.env, default to 'env'
    - [ ] Load backend-specific config based on KEY_BACKEND value
    - [ ] Throw error if required variables missing (e.g., KEY_BACKEND='aws-kms' but AWS_KMS_EVM_KEY_ID not set)
    - [ ] [Source: docs/architecture/core-workflows.md lines 97-98 config loading pattern]
  - [ ] Create .env.example template
    - [ ] File: `.env.example` in project root
    - [ ] Include all KEY\_\* variables with comments explaining each
    - [ ] Example values for each backend type
    - [ ] Security warnings: "NEVER commit .env file to version control"
    - [ ] [Source: docs/architecture/security.md lines 26-44]
  - [ ] Add configuration validation
    - [ ] Function: `validateKeyManagerConfig(config: KeyManagerConfig): void`
    - [ ] Validate required fields present for selected backend
    - [ ] Validate key ID formats (ARN for AWS, resource name for GCP, URL for Azure)
    - [ ] Throw ConfigurationError if validation fails
    - [ ] [Source: docs/architecture/error-handling-strategy.md lines 15-35]
  - [ ] Unit test: Verify config loaded from environment variables
  - [ ] Unit test: Verify error thrown if required variable missing
  - [ ] Unit test: Verify config validation catches invalid key ID formats

- [ ] Task 9: Implement Unit Tests with Mocked KMS Backends (AC: 9)
  - [ ] Create unit test file
    - [ ] File: `packages/connector/test/unit/key-manager.test.ts`
    - [ ] Import KeyManager, all backend classes, mock factories
    - [ ] [Source: docs/architecture/test-strategy-and-standards.md lines 18-60]
  - [ ] Setup test fixtures (beforeEach)
    - [ ] Mock AWS KMS client: `{ send: jest.fn().mockResolvedValue({ Signature: Buffer.from('...'), PublicKey: Buffer.from('...'), KeyMetadata: { KeyId: 'new-key-id' } }) }`
    - [ ] Mock GCP KMS client: `{ asymmetricSign: jest.fn().mockResolvedValue({ signature: Buffer.from('...') }), getPublicKey: jest.fn().mockResolvedValue({ publicKey: '...' }), createCryptoKeyVersion: jest.fn().mockResolvedValue({ name: '...' }) }`
    - [ ] Mock Azure Key Vault client: `{ sign: jest.fn().mockResolvedValue({ result: Buffer.from('...') }), getKey: jest.fn().mockResolvedValue({ key: { kid: '...', n: '...' } }), createKey: jest.fn().mockResolvedValue({ name: '...' }) }`
    - [ ] Mock HSM PKCS#11: `{ C_Sign: jest.fn().mockReturnValue({ rv: CKR_OK, signature: Buffer.from('...') }), C_GetAttributeValue: jest.fn().mockReturnValue({ rv: CKR_OK, attributes: [{ type: CKA_VALUE, value: Buffer.from('...') }] }), C_GenerateKeyPair: jest.fn().mockReturnValue({ rv: CKR_OK, publicKey: 1, privateKey: 2 }) }`
    - [ ] Mock Logger: `{ info: jest.fn(), warn: jest.fn(), error: jest.fn(), child: jest.fn().mockReturnThis() }`
    - [ ] [Source: docs/architecture/test-strategy-and-standards.md lines 36-59]
  - [ ] Test backend selection (6 tests)
    - [ ] Test: `should select EnvironmentVariableBackend when backend='env'`
    - [ ] Test: `should select AWSKMSBackend when backend='aws-kms'`
    - [ ] Test: `should select GCPKMSBackend when backend='gcp-kms'`
    - [ ] Test: `should select AzureKeyVaultBackend when backend='azure-kv'`
    - [ ] Test: `should select HSMBackend when backend='hsm'`
    - [ ] Test: `should throw error for unknown backend type`
    - [ ] [Source: Epic 12 Story 12.2 testing requirements]
  - [ ] Test sign method (6 tests)
    - [ ] Test: `should delegate sign() to backend`
    - [ ] Test: `should log SIGN_REQUEST audit event before signing`
    - [ ] Test: `should log SIGN_SUCCESS audit event after successful signing`
    - [ ] Test: `should log SIGN_FAILURE audit event on signing error`
    - [ ] Test: `should return signature buffer from backend`
    - [ ] Test: `should throw error if backend.sign() fails`
  - [ ] Test getPublicKey method (3 tests)
    - [ ] Test: `should delegate getPublicKey() to backend`
    - [ ] Test: `should return public key buffer from backend`
    - [ ] Test: `should throw error if key not found`
  - [ ] Test key rotation (6 tests)
    - [ ] Test: `should initiate key rotation via backend.rotateKey()`
    - [ ] Test: `should log KEY_ROTATION_START audit event before rotation`
    - [ ] Test: `should log KEY_ROTATION_COMPLETE audit event after rotation`
    - [ ] Test: `should return new key ID from backend`
    - [ ] Test: `should maintain overlap period (old and new key both valid)`
    - [ ] Test: `should notify before rotation (14 days warning)`
  - [ ] Run tests with stability validation (10x runs)
    - [ ] Execute: `for i in {1..10}; do npm test -- key-manager.test.ts || echo "Run $i FAILED"; done`
    - [ ] Verify 10/10 runs pass (100% success rate)
    - [ ] [Source: docs/architecture/test-strategy-and-standards.md lines 729-786]

- [ ] Task 10: Implement AWS KMS Integration Test (AC: 10)
  - [ ] Create integration test file
    - [ ] File: `packages/connector/test/integration/aws-kms-signing.test.ts`
    - [ ] Import KeyManager, ethers, xrpl verification functions
    - [ ] [Source: docs/architecture/test-strategy-and-standards.md lines 62-133]
  - [ ] Setup AWS credentials check (beforeAll)
    - [ ] Check environment variables: `process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY && process.env.AWS_KMS_EVM_KEY_ID && process.env.AWS_KMS_XRP_KEY_ID`
    - [ ] If credentials not available: skip test with message "AWS credentials not configured. Set AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_KMS_EVM_KEY_ID, AWS_KMS_XRP_KEY_ID to run this test."
    - [ ] Use `describeIf()` helper or Jest `describe.skip()`
    - [ ] CI/CD Strategy: Integration tests will be skipped in CI unless GitHub Actions secrets are configured (AWS_KMS_TEST_KEY_ID, etc.). Local development should use personal AWS test accounts.
    - [ ] [Source: Epic 12 Story 12.2 AC 10 conditional test execution]
  - [ ] Test: EVM signing with real AWS KMS key
    - [ ] Test name: `should sign EVM message with real AWS KMS key`
    - [ ] Create KeyManager with backend='aws-kms', config from environment variables
    - [ ] Create test message: `Buffer.from('test-message-for-evm-signing')`
    - [ ] Sign message: `const signature = await keyManager.sign(testMessage, process.env.AWS_KMS_EVM_KEY_ID!)`
    - [ ] Get public key: `const pubKey = await keyManager.getPublicKey(process.env.AWS_KMS_EVM_KEY_ID!)`
    - [ ] Verify signature using ethers: `const recoveredAddress = ethers.verifyMessage(testMessage.toString(), signature.toString('hex'))`
    - [ ] Assert: Recovered address matches expected address derived from public key
    - [ ] [Source: Epic 12 Story 12.2 AC 10 EVM signing verification]
  - [ ] Test: XRP claim signing with real AWS KMS key
    - [ ] Test name: `should sign XRP claim with real AWS KMS key`
    - [ ] Create KeyManager with backend='aws-kms'
    - [ ] Create test claim: `channelId = '0123456789ABCDEF...', amount = '1000000'`
    - [ ] Create claim message: `const message = createClaimMessage(channelId, amount)`
    - [ ] Sign claim: `const signature = await keyManager.sign(message, process.env.AWS_KMS_XRP_KEY_ID!)`
    - [ ] Get public key: `const pubKey = await keyManager.getPublicKey(process.env.AWS_KMS_XRP_KEY_ID!)`
    - [ ] Verify signature using xrpl: `const isValid = verifyPaymentChannelClaim(channelId, amount, signature.toString('hex'), pubKey.toString('hex'))`
    - [ ] Assert: Signature is valid (isValid === true)
    - [ ] [Source: Epic 12 Story 12.2 AC 10 XRP signing verification]
  - [ ] Test: Key rotation with AWS KMS
    - [ ] Test name: `should rotate key and maintain overlap period`
    - [ ] Create KeyManager with backend='aws-kms'
    - [ ] Rotate key: `const newKeyId = await keyManager.rotateKey(process.env.AWS_KMS_EVM_KEY_ID!)`
    - [ ] Assert: New key ID returned
    - [ ] Sign with new key: Verify signature works with new key ID
    - [ ] Sign with old key (during overlap): Verify signature still works with old key ID (if within overlap period)
    - [ ] [Source: Epic 12 Story 12.2 AC 6, AC 10 key rotation integration test]
  - [ ] Cleanup (afterAll)
    - [ ] Note: Do NOT delete AWS KMS keys in test (expensive operation, manual cleanup recommended)
    - [ ] Log warning: "AWS KMS keys created during tests are not automatically deleted. Clean up manually in AWS Console."
    - [ ] [Source: AWS KMS best practices]
...
(To be filled in by QA after testing)
