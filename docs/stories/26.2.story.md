...
- [x] Task 1: Configure `@agent-society/shared` package.json for publishing (AC: 1, 2, 3)
  ...
  - [x] 1.13 Create `packages/shared/README.md` with the following structure:
    ...
    - Install: `npm install @agent-society/shared`
...
- [x] Task 2: Configure `@agent-society/connector` package.json for publishing (AC: 1, 2, 3, 4)
  - [x] 2.1 Version bump: change `"version"` from `"0.1.0"` to `"1.0.0"`
  - [x] 2.2 Update `"@agent-society/shared"` dependency from `"*"` to `"^1.0.0"` — this is critical for npm publishing; workspace protocol (`*`) cannot be resolved outside the monorepo
...
- [x] Task 4: Verify `npm pack` produces clean tarballs (AC: 5, 6)
  - [x] 4.1 Run `npm pack --workspace=@agent-society/shared --dry-run` and verify output lists only files under `dist/`, `README.md`, `LICENSE`, and `package.json`
  - [x] 4.2 Run `npm pack --workspace=@agent-society/connector --dry-run` and verify output lists only files under `dist/`, `README.md`, `LICENSE`, and `package.json`
...
- [x] Task 5: Verify all existing tests still pass (AC: 7)
  - [x] 5.1 Run `npm test --workspace=@agent-society/shared` — all 170 tests pass (5 suites)
  - [x] 5.2 Run `npm test --workspace=@agent-society/connector` — 93 suites pass, 20 skipped, 42 fail from pre-existing missing optional/peer deps (identical to base branch)
...
- [x] Task 6: Verify workspace development still works (AC: 4)
  ...
  - [x] 6.2 Verify the connector can still import from shared package: `import { PacketType } from '@agent-society/shared'` resolves to local workspace package
...
This is the second story in Epic 26 (npm Publishing Readiness). It configures both `@agent-society/shared` and `@agent-society/connector` with proper npm publishing metadata so they can be published as scoped public packages.
...
- Reduced direct dependencies from 42 to **6**: `@agent-society/shared`, `js-yaml`, `pino`, `tslib`, `ws`, `zod`
...
**`@agent-society/shared` (`packages/shared/package.json`):**
...
**`@agent-society/connector` (`packages/connector/package.json`):**
...
- `"@agent-society/shared": "*"` — needs updating to `"^1.0.0"` for npm publishing
...
When `@agent-society/connector` depends on `@agent-society/shared` at `"^1.0.0"`, npm workspaces will still resolve it to the local workspace package during development (as long as the local package version satisfies the range). This is standard npm workspace behavior. The `"*"` → `"^1.0.0"` change is only necessary for published packages to have a valid semver range that npm registry consumers can resolve.
...
- **Key validation commands:**
  - `npm test --workspace=@agent-society/shared` — shared package tests
  - `npm test --workspace=@agent-society/connector` — connector tests (specifically `lib.test.ts`, `consumer-types.test.ts`)
  ...
  - `npm pack --workspace=@agent-society/shared --dry-run` — verify shared tarball contents
  - `npm pack --workspace=@agent-society/connector --dry-run` — verify connector tarball contents
...
5. **Workspace dependency change** — Changing `"@agent-society/shared": "*"` to `"^1.0.0"` should not break workspace development. npm resolves workspace packages by name matching, and the local version (`1.0.0` after bump) satisfies `^1.0.0`.
...
- QA fix validation: `npm test --workspace=@agent-society/shared` — 170/170 pass after fixes

### Completion Notes

- Both `@agent-society/shared` and `@agent-society/connector` configured with full npm publishing metadata
...
- `@agent-society/shared` dependency in connector changed from `"*"` to `"^1.0.0"` for npm registry resolution
...
- Workspace resolution verified: `@agent-society/shared` resolves to local package from connector
...
| 4   | Connector depends on shared at ^1.0.0                     | `packages/connector/package.json:80` — `"@agent-society/shared": "^1.0.0"`. Workspace resolution verified via `npm ls`                                                     | ✓      |
