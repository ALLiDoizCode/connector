...
- [x] Task 6: Verify TypeScript compilation (AC: 6)
  ...
  - [x] 6.2 Run `npm run build:connector-only --workspace=@agent-society/connector` to verify TypeScript compiles successfully
  - [x] 6.3 Verify `dist/lib.js`, `dist/lib.d.ts`, `dist/main.js` all exist in build output
  - [x] 6.4 Run `npm run build --workspace=@agent-society/shared && npm run build:connector-only --workspace=@agent-society/connector` for full workspace build
...
- [x] Task 8: Run full test suite and verify no regressions (AC: 7)
  - [x] 8.1 Run `npm test --workspace=@agent-society/connector` — all existing tests must pass
  - [x] 8.2 Run `npm test --workspace=@agent-society/shared` — shared package unaffected
...
This is the first story in Epic 25 (CLI/Library Separation & Lifecycle Cleanup). Epic 25 depends on Epic 24 (Connector Library API), which is fully complete (stories 24.1-24.4 all Done). The goal of Epic 25 is to make `@agent-society/connector` safe to import and embed without side effects — separating the CLI/process lifecycle from the library exports.
...
| --- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Import from `@agent-society/connector` uses `lib.ts` — zero side effects | `lib.test.ts`: verifies ConnectorNode, createLogger, ConfigLoader exported; verifies `main` NOT exported. Static analysis confirms zero `process` references in `lib.ts`.            |
