...
docker build -t ilp-connector packages/connector
...
- Integration tests created but deferred due to pre-existing jest configuration issues with @m2m/shared module resolution
- This is a project-wide issue affecting all existing tests, not introduced by this story
- Resolution: Added @m2m/shared as dependency to connector package.json to enable proper module resolution
