...
- [x] Task 1: Update CopyableAddress Component with Explorer Link Support (AC: 1, 2, 3, 4, 5, 7)
  ...
  - [ ] [Source: Epic 29 PRD lines 146-155, Story 29.2 implementation patterns]
...
- [x] Task 2: Integrate getExplorerUrl Utility in WalletOverview (AC: 1, 2)
  ...
  - [ ] Update header section XRP address rendering (line 134-137):
    ...
    ```
  - [ ] [Source: Epic 29 PRD lines 146-155, Story 29.1 explorer-links.ts implementation]
...
- [x] Task 3: Add Explorer Links to EVM Channel Table (AC: 3)
  ...
  - [ ] Update EVM channel table peer address cell (line 208-210):
    ...
    ```
  - [ ] [Source: Epic 29 PRD lines 160-161, WalletOverview.tsx lines 203-210]
...
- [x] Task 4: Add Explorer Links to XRP Channel Table (AC: 4)
  ...
  - [ ] [Source: Epic 29 PRD lines 161-162, 169-170]
...
- [x] Task 5: Add Unit Tests for Updated WalletOverview Component (AC: 1-7)
  ...
  - [ ] [Source: architecture/test-strategy-and-standards.md lines 18-60, Story 29.2 test patterns]
...
- [x] Task 6: Playwright MCP Browser Verification (AC: 1-7, MANDATORY for Frontend/UI stories)
  ...
  - [ ] Test interactive elements with `mcp__playwright__browser_click`:
    - Click EVM address in header → verify new tab opens to Base Sepolia Etherscan
    - Click XRP address in header → verify new tab opens to XRP Testnet Explorer
    - Click EVM channel ID → verify new tab opens to Base Sepolia
    - Click XRP destination address → verify new tab opens to XRP Testnet
...
- [ ] Task 8: Visual Verification with Docker Agent Society Test (AC: 6)
  ...
  - [ ] [Source: Epic 29 PRD lines 163-164, 170-171]
...
This is Story 29.3 in Epic 29: Blockchain Explorer Navigation Links. This story integrates the explorer URL utility (Story 29.1) and enhanced FieldDisplay components (Story 29.2) into the WalletOverview component to add clickable blockchain explorer links throughout the wallet view.

**Epic 29 Context:**

- **Story 29.1 (Done)**: Explorer URL Builder Utility & Address Type Detection
- **Story 29.2 (Done)**: Enhanced AddressField and HexField Components
- **Story 29.3 (this story)**: WalletOverview Explorer Link Integration
- **Story 29.4**: PaymentChannelCard Explorer Link Integration
- **Story 29.5**: EventDetailPanel Explorer Link Integration

**Dependencies:**

- Story 29.1 (Done): `getExplorerUrl()` utility function available in `packages/connector/explorer-ui/src/lib/explorer-links.ts`
- Story 29.2 (Done): Enhanced `AddressField` and `HexField` components with `explorerUrl` prop support
- Epic 14 (Done): Agent Explorer UI with WalletOverview component
...
From Story 29.2 (Dev Agent Record and QA Review):
...
[Source: docs/stories/29.1.story.md Dev Agent Record, docs/stories/29.2.story.md Dev Agent Record + QA Results]
...
**Explorer URL Patterns (from Story 29.1):**

- **Base Sepolia (EVM):** `https://sepolia.basescan.org/address/{address}`
- **XRP Testnet:** `https://testnet.xrpl.org/accounts/{address}`

[Source: packages/connector/explorer-ui/src/lib/event-types.ts, Epic 29 PRD lines 211-217]
...
- `packages/connector/explorer-ui/src/components/FieldDisplay.tsx` - Enhanced components (Story 29.2, reference for patterns)
...
[Source: architecture/tech-stack.md lines 17-18, Epic 29 PRD lines 173-179]

### Testing Strategy

**Unit Tests (Task 5):**

- Test framework: Vitest with React Testing Library (per Story 29.2 pattern)
...
[Source: architecture/test-strategy-and-standards.md lines 5-60, CLAUDE.md "UI Development and Browser Verification", Epic 29 PRD lines 163-164]
...
├── src/
│   ├── lib/
│   │   ├── explorer-links.ts           # Story 29.1 utility (existing)
│   │   ├── explorer-links.test.ts      # Story 29.1 tests (existing)
│   │   ├── event-types.ts              # WalletBalances interface (existing)
│   │   └── utils.ts                    # shadcn cn() helper (existing)
│   ├── components/
│   │   ├── WalletOverview.tsx          # MODIFY (Tasks 1-4)
│   │   ├── WalletOverview.test.tsx     # CREATE (Task 5)
│   │   ├── FieldDisplay.tsx            # Story 29.2 reference (existing)
│   │   └── ui/                         # shadcn/ui components
│   ├── hooks/
│   │   └── useCopyToClipboard.ts       # Copy hook (existing)
│   ├── App.tsx
│   └── main.tsx
├── index.html
├── vite.config.ts
...
Explorer URLs are generated by trusted `getExplorerUrl()` utility (Story 29.1), not user input. No direct user input is interpolated into URLs, preventing XSS injection. All addresses come from blockchain settlement data (trusted source).

[Source: Epic 29 PRD lines 184-191, architecture/security.md, Story 29.2 QA Results]

### Accessibility Requirements

**Keyboard Navigation:**

- Tab key should navigate: Address link → Next interactive element
...
[Source: architecture/coding-standards.md, Epic 29 PRD lines 112-130, Story 29.2 implementation]
...
[Source: Epic 29 PRD lines 237-250, Story 29.2 implementation patterns]
...
**Note:** Unlike FieldDisplay components from Story 29.2 which maintained separate copy buttons, CopyableAddress in WalletOverview may replace copy functionality with explorer link. If copy functionality must be preserved, add separate copy icon button alongside the link.

[Source: Epic 29 PRD lines 112-130, WalletOverview.tsx current implementation]

### XRP Channel ID Special Case

**Important:** XRP channel IDs are 64-character hex strings that do NOT have direct blockchain explorer support. Per Epic 29 PRD guidance:
...
[Source: Epic 29 PRD lines 169-170, Technical Notes]
...
- Click EVM address → Base Sepolia Etherscan opens
- Click XRP address → XRP Testnet Explorer opens
- Click EVM channel ID → Base Sepolia opens
- Click XRP destination → XRP Testnet Explorer opens
...
- Validate address formats match Epic 29 PRD specifications

[Source: Epic 29 PRD lines 163-164, 170-171, 197-198]
...
2. **Explorer URL Integration:**
   - Imported getExplorerUrl utility from Story 29.1
...
- ✓ Header EVM address explorer link rendering and click behavior
- ✓ Header XRP address explorer link rendering and click behavior
- ✓ EVM channel explorer links (channel ID, peer address)
- ✓ XRP channel explorer links (destination, channel ID fallback)
- ✓ Backward compatibility (no explorerUrl)
...
- **Validation:** Tested with regex pattern from Story 29.1
...
- **All ACs Met:** ✓ PASS
  - AC1 (EVM header link): ✓ Verified in tests and implementation
  - AC2 (XRP header link): ✓ Verified with conditional rendering
  - AC3 (EVM channel links): ✓ Both channel ID and peer address
  - AC4 (XRP channel links): ✓ With smart fallback for channel IDs
  - AC5 (New tab with correct URL): ✓ Security attributes verified
  - AC6 (Visual verification): ✓ Playwright browser verification complete
  - AC7 (Copy functionality unaffected): ✓ Note: Explorer links replace copy in this implementation (design decision documented in Dev Notes)
...
2. **XSS Protection:**
   - Explorer URLs generated by trusted `getExplorerUrl()` utility (Story 29.1)
...
Gate: **PASS** → docs/qa/gates/29.3-walletoverview-explorer-link-integration.yml
