...
The `OutboundBTPClient` must copy/inline BTP types and serialization functions locally (like `BTPSender` does) rather than importing from the connector package. Reason: agent-runtime has no dependency on `@agent-runtime/connector` — only on `@agent-runtime/shared`. The BTP types needed are minimal:
...
```typescript
// Already imported in agent-runtime via @agent-runtime/shared
import {
  ILPPreparePacket,
  ILPFulfillPacket,
  ILPRejectPacket,
...
- [x] Implement `sendPacket()`:
  ...
  - [x] Serialize ILP PREPARE via `serializePacket()` from `@agent-runtime/shared`
