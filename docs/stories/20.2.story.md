...
The `OutboundBTPClient` must copy/inline BTP types and serialization functions locally (like `BTPSender` does) rather than importing from the connector package. Reason: agent-runtime has no dependency on `@agent-society/connector` — only on `@agent-society/shared`. The BTP types needed are minimal:
...
```typescript
// Already imported in agent-runtime via @agent-society/shared
import {
  ILPPreparePacket,
  ILPFulfillPacket,
  ILPRejectPacket,
...
| `CONNECTOR_BTP_PEER_ID`    | No       | `agent-runtime` | Peer ID used during BTP authentication handshake.                                                                               |
...
- [x] Implement `sendPacket()`:
  ...
  - [x] Serialize ILP PREPARE via `serializePacket()` from `@agent-society/shared`
