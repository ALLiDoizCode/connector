...
- [x] Task 2: Add `sendPacket()` method to `ConnectorNode` (AC: 1, 2, 3, 4, 5)
  ...
  - [x] 2.2 Import the necessary types at the top of `connector-node.ts`:
    ...
    - `ILPPreparePacket`, `ILPFulfillPacket`, `ILPRejectPacket`, `PacketType`, `ILPErrorCode` from `@agent-society/shared`
...
- [x] Task 3: Update `packages/connector/src/index.ts` exports (AC: 1, 5)
  ...
  - [x] 3.2 Export `ConnectorNotStartedError` class from `index.ts`:
    ...
    ```
  - [x] 3.3 Re-export `ILPFulfillPacket`, `ILPRejectPacket` types from `@agent-society/shared` for convenience — library consumers need these to type-check the `sendPacket()` return value. Add to the type exports block:
    ```typescript
    export type { ILPFulfillPacket, ILPRejectPacket } from '@agent-society/shared';
    ```
    **Note**: `@agent-society/shared` already exports `ILPFulfillPacket` and `ILPRejectPacket` (lines 18-19 of `packages/shared/src/index.ts`). This re-export is a convenience so library consumers can import everything from `@agent-society/connector` without adding `@agent-society/shared` as a direct dependency.
