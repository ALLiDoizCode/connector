...
`isValidILPAddress()` is exported from `@agent-society/shared` and used in admin-api.ts for route prefix validation. **Not yet imported in connector-node.ts** — add it to the existing `@agent-society/shared` import alongside `ILPAddress`:
