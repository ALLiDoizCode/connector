...
describe('TelemetryEmitter Integration Tests', () => {
  ...
  describe('XRP Channel Telemetry (Story 9.7)', () => {
    let emitter: TelemetryEmitter;
    ...
    it('should emit XRP_CHANNEL_OPENED with correct fields', async () => {
      // Arrange
      const channelState = {
      ...
      // Assert
      expect(testServer.receivedMessages.length).toBe(1);
      const event = testServer.receivedMessages[0]!;
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.objectContaining({ channelId: 'A'.repeat(64), peerId: 'peer-bob' }),
        'XRP_CHANNEL_OPENED telemetry emitted'
      );
    });

    it('should emit XRP_CHANNEL_OPENED without peerId', async () => {
      // Arrange
      const channelState = {
        ...
        balance: '0',
        settleDelay: 43200,
        publicKey: 'ED' + 'D'.repeat(64),
        status: 'open' as const,
      };

      // Act
      emitter.emitXRPChannelOpened(channelState);

      // Wait for message to be received
      await waitFor(() => testServer.receivedMessages.length > 0, 1000);

      // Assert
      const event = testServer.receivedMessages[0]! as unknown as XRPChannelOpenedEvent;
      expect(event.type).toBe('XRP_CHANNEL_OPENED');
      expect(event.peerId).toBeUndefined();
    });

    it('should emit XRP_CHANNEL_CLAIMED with claim amounts', async () => {
      // Arrange
      const channelId = 'A'.repeat(64);
      const claimAmount = '5000000000';
      const remainingBalance = '5000000000';

      // Act
      emitter.emitXRPChannelClaimed(channelId, claimAmount, remainingBalance, 'peer-bob');
    ...
    it('should emit XRP_CHANNEL_CLOSED with cooperative closeType', async () => {
      // Arrange
      const channelId = 'A'.repeat(64);
      const finalBalance = '5000000000';
      const closeType = 'cooperative';

      // Act
      emitter.emitXRPChannelClosed(channelId, finalBalance, closeType, 'peer-bob');
    ...
    it('should emit XRP_CHANNEL_CLOSED with all closeType values', async () => {
      // Test all three closeType values
      const closeTypes: Array<'cooperative' | 'expiration' | 'unilateral'> = [
        'cooperative',
        'expiration',
        'unilateral',
      ];
      ...
      for (const closeType of closeTypes) {
        ...
        // Act
        emitter.emitXRPChannelClosed('C'.repeat(64), '1000000000', closeType);

        // Wait for message
        await waitFor(() => testServer.receivedMessages.length > 0, 1000);

        // Assert
        const event = testServer.receivedMessages[0]! as unknown as XRPChannelClosedEvent;
        expect(event.type).toBe('XRP_CHANNEL_CLOSED');
        expect(event.closeType).toBe(closeType);
      }
    });
  });
});
