...
/**
 * Derive XRP wallet from master seed
 * @param masterSeed - Master seed buffer (512 bits)
 * @param index - Wallet index
 * @returns Derived wallet with XRP address
 */
function deriveXRPWallet(masterSeed: Buffer, index: number): DerivedWallet {
  // Derive HD key using BIP-44 path for XRP
  const path = `${DERIVATION_PATHS.XRP}/${index}`;
  const hdKey = HDKey.fromMasterSeed(masterSeed).derive(path);
  ...
  // Convert private key to XRP wallet using fromEntropy
...
describeIfLocal('HD Wallet Derivation Integration Tests', () => {
  ...
  describe('XRP Address Derivation', () => {
    it('should derive 1000 unique XRP addresses from single master seed', async () => {
      // Generate master seed
      const masterSeed = await manager.generateMasterSeed(256);

      // Derive 1000 XRP addresses
      const startTime = Date.now();
      const addresses: string[] = [];

      for (let i = 0; i < 1000; i++) {
        const wallet = deriveXRPWallet(masterSeed.seed, i);
        addresses.push(wallet.xrpAddress!);
      }
    ...
    it('should derive same XRP addresses from same master seed (deterministic)', async () => {
      ...
      // Derive wallet at index 0 from both seeds
      const wallet1 = deriveXRPWallet(masterSeed1.seed, 0);
      const wallet2 = deriveXRPWallet(masterSeed2.seed, 0);
    ...
    it('should derive different addresses for different indices', async () => {
      ...
      const wallet0 = deriveXRPWallet(masterSeed.seed, 0);
      const wallet1 = deriveXRPWallet(masterSeed.seed, 1);
      const wallet100 = deriveXRPWallet(masterSeed.seed, 100);

      // Verify all addresses are different
      expect(wallet0.xrpAddress).not.toBe(wallet1.xrpAddress);
      expect(wallet0.xrpAddress).not.toBe(wallet100.xrpAddress);
      expect(wallet1.xrpAddress).not.toBe(wallet100.xrpAddress);
    ...
    it('should generate valid XRP addresses (r prefix)', async () => {
      const masterSeed = await manager.generateMasterSeed(256);
      const wallet = deriveXRPWallet(masterSeed.seed, 0);

      // Verify address format
      expect(wallet.xrpAddress).toBeDefined();
      expect(wallet.xrpAddress!.startsWith('r')).toBe(true);
      expect(wallet.xrpAddress!.length).toBeGreaterThanOrEqual(25);
      expect(wallet.xrpAddress!.length).toBeLessThanOrEqual(35);
    });
  });

  describe('Cross-Chain Derivation', () => {
    it('should derive different addresses for EVM and XRP from same master seed', async () => {
      const masterSeed = await manager.generateMasterSeed(256);

      const evmWallet = deriveEVMWallet(masterSeed.seed, 0);
      const xrpWallet = deriveXRPWallet(masterSeed.seed, 0);

      // Verify addresses are different (different derivation paths)
      expect(evmWallet.evmAddress).toBeDefined();
      expect(xrpWallet.xrpAddress).toBeDefined();
      expect(evmWallet.evmAddress).not.toBe(xrpWallet.xrpAddress);

      // Verify private keys are different (different derivation paths)
      expect(evmWallet.privateKey.equals(xrpWallet.privateKey)).toBe(false);
    });

    it('should support deriving both EVM and XRP wallets from same master seed', async () => {
      const masterSeed = await manager.generateMasterSeed(256);

      // Derive 10 of each type
      const evmAddresses: string[] = [];
      const xrpAddresses: string[] = [];

      for (let i = 0; i < 10; i++) {
        evmAddresses.push(deriveEVMWallet(masterSeed.seed, i).evmAddress!);
        xrpAddresses.push(deriveXRPWallet(masterSeed.seed, i).xrpAddress!);
      }

      // Verify all EVM addresses unique
      expect(new Set(evmAddresses).size).toBe(10);

      // Verify all XRP addresses unique
      expect(new Set(xrpAddresses).size).toBe(10);

      // Verify no overlap between EVM and XRP addresses
      const allAddresses = [...evmAddresses, ...xrpAddresses];
      expect(new Set(allAddresses).size).toBe(20);
    });
  });
  ...
  describe('Master Seed Persistence', () => {
    it('should derive same addresses after encrypting and decrypting master seed', async () => {
      ...
      const xrpBefore = deriveXRPWallet(originalSeed.seed, 0);
      ...
      const xrpAfter = deriveXRPWallet(restoredSeed.seed, 0);

      // Verify addresses match
      expect(evmAfter.evmAddress).toBe(evmBefore.evmAddress);
      expect(xrpAfter.xrpAddress).toBe(xrpBefore.xrpAddress);
    });
  ...
  describe('Performance', () => {
    it('should derive 1000 wallets in reasonable time (<15s)', async () => {
      ...
      // Derive 1000 wallets (mix of EVM and XRP)
      for (let i = 0; i < 500; i++) {
        deriveEVMWallet(masterSeed.seed, i);
        deriveXRPWallet(masterSeed.seed, i);
      }
